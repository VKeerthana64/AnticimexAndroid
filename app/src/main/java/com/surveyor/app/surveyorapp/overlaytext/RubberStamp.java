...
package com.surveyor.app.surveyorapp.overlaytext;
...
import static com.surveyor.app.surveyorapp.overlaytext.RubberStampPosition.CUSTOM;
import static com.surveyor.app.surveyorapp.overlaytext.RubberStampPosition.TILE;
...
public class RubberStamp {
    ...
    private void addTextToBitmap(@NonNull RubberStampConfig config,
        ...
        String[] arr = rubberStampString.split("-");
        //paint.getTextBounds(rubberStampString,0,rubberStampString.length(),bounds);
        paint.getTextBounds(arr[0], 0, arr[0].length(), bounds);
        paint1 = paint;
        paint1.getTextBounds(arr[1], 0, arr[1].length(), bounds1);

        int rubberStampWidth = bounds.width();
        int rubberStampWidth1 = bounds1.width();
        float rubberStampMeasuredWidth = paint.measureText(rubberStampString);
        int rubberStampHeight = bounds.height();
        int rubberStampHeight1 = bounds1.height();

        int positionX = config.getPositionX();
        int positionY = config.getPositionY();
  int positionX1 = config.getPositionX();
        int positionY1 = config.getPositionY();

        if (config.getRubberStampPosition() != CUSTOM) {
            // If the specified RubberStampPosition is not CUSTOM, use calculates its x & y
            // co-ordinates.
            Pair<Integer, Integer> pair = PositionCalculator.getCoordinates(config.getRubberStampPosition(),
                    baseBitmapWidth, baseBitmapHeight,
                    rubberStampWidth, rubberStampHeight);
           Pair<Integer, Integer> pair1 = PositionCalculator.getCoordinates(config.getRubberStampPosition(),
                    baseBitmapWidth, baseBitmapHeight,
                    rubberStampWidth1, rubberStampHeight1);
            positionX = pair.first;
            positionY = pair.second;
            positionX1 = pair1.first;
            positionY1 = pair1.second;
        }

        // Add the margin to this position if it was passed to the config.
        positionX += config.getXMargin();
        positionY += config.getYMargin();
        positionX1 += config.getXMargin();
        positionY1 += config.getYMargin();
        ...
        if (config.getRubberStampPosition() != TILE) {
            ...
            canvas.drawText(arr[0], positionX+50, positionY, paint);
            canvas.drawText(arr[1], positionX+50, positionY+25, paint1);
        } else {
